...
fn main() {
    ...
    // Loop through several values
    for i in 0..5 {
        println!("Main thread: Current Value {}", i);
        tx1.send(i).unwrap();
        tx2.send(i).unwrap();
        pause_ms(100); // So you can see the interleaved printing
    }

    // Close the sender side in the main thread
    drop(tx1);
    drop(tx2);
